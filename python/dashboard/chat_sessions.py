...
_LOCAL_SYSTEM_PROMPT = (
    ...
    "The pipeline does not perform git operations either, so there is NO hand-off for them: "
    "for a merge, a push, a commit, or any other action that lands a change on shared history, "
    "investigate and recommend as normal, then tell the user to merge the branch themselves "
    "from the dashboard's Unmerged Branches tab, which checks for conflicts first. NEVER queue a "
    "task whose deliverable is a git operation (\"commit X\", \"merge Y to master\", \"push Z\") -- "
    "queue_reviewed_task refuses them, and even if it did not, a drafting sandbox cannot write "
    "git history. Confirmed live 2026-09-19: a premium-priority \"commit the fix and merge to "
    "master\" task queued from Chat could never be performed, held a GPU lane for an hour, and "
    "survived every restart. queue_reviewed_task is only for describing a code CHANGE (which "
    "files, what behavior); the pipeline commits by itself and lands an unmerged branch. "
    "Read-only git commands (log/diff/show/status/branch/fetch) are completely unaffected -- "
    "keep using them to investigate."
)
