...
function renderDraftAttempts(task) {
  ...
  const one = (a) => {
    ...
    const summary = `<summary>#${a.attemptNo} · <span class="da-outcome-${escapeHtml(outcome)}">${escapeHtml(outcome)}</span>`
      + `${a.blockedReason ? ` — ${escapeHtml(String(a.blockedReason).slice(0, 120))}` : ''}`
...
function renderSubTaskChecklist(task) {
  ...
  const rows = subs.map((st) => {
    ...
    return `<div class="task-history-row"><span class="badge ${cls}">${escapeHtml(label)}</span> `
      + `${taskLink(st.id)} ${escapeHtml(st.title || '')}</div>`;
  }).join('');
...
function renderTaskDetailModal(task) {
  ...
  if (PROSE_REPORT_SOURCES.has(task.source) && task.implementResponse) {
    html += `<div class="field-label">Root-Cause Report</div><pre>${escapeHtml(task.implementResponse)}</pre>`;
    reportShown = true;
  }
  ...
  if (task.implementResponse && !reportShown) html += `<div class="field-label">Implement</div><pre>${escapeHtml(task.implementResponse)}</pre>`;
...
async function renderBranchDetailModal(b) {
  ...
  if (hub) {
    ...
    html += `<div class="meta">${escapeHtml(hub.title || hub.id)} · ${p.done}/${p.total} task(s) done`
      + (gate ? ` · integration gate <strong>${escapeHtml(gate)}</strong>` : '') + `</div>`;
    if (hub.blockedReason) html += `<div class="meta" style="color:var(--bad);margin-top:4px">${escapeHtml(hub.blockedReason)}</div>`;
    html += `<div class="task-history" style="margin-top:6px">` + (hub.subTasks || []).map((st) =>
      `<div class="task-history-row"><span class="task-history-stage">${escapeHtml(st.title || st.id)}</span> `
