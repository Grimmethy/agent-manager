...
| `pipeline_forensics` | 44 | the deep sibling of `pipeline_self_audit` (`pipeline-forensics.js` + `forensic-bundle.js`). Triggered by a `needs-clarification` cluster, a low-shipped-value task source, or an on-demand `queue/forensics-requests/*.json` request. Assembles a full evidence bundle -- every `draftAttempts` tier, the worklog transcript, `model_calls` rows, **plus a contrast set of sibling tasks that succeeded** -- and drafts a ranked root-cause report (advisory prose). Held at `queue/awaiting-confirm/` for a human, then files a fix as a `### AC-NNN` candidate in `Docs/PIPELINE_FIX_CANDIDATES.md`. Coverage in `AGENT_MANAGER_FORENSICS_COVERAGE_PATH` |
| `pipeline_forensics_fix` | 43 | `Docs/PIPELINE_FIX_CANDIDATES.md` -- turns a confirmed `pipeline_forensics` fix candidate into a real `src/` diff on a pushed `agent/*` branch for manual merge |
