...
| **S5e** | The actual kernel move + freeze window: relocate the files listed in S5's own row behind S5a-S5d's hooks, flip `queue-watcher.sh`'s two sweep invocations to the plugin path via S5a's resolver. One atomic cutover -- `coordinator-sweep.js` is the single reconciliation loop for every in-flight hub, so this cannot be split further. | S5a, S5b, S5c, S5d, S4, S0 |
