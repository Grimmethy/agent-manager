...
| **S5c** | New swap point: `review-task.js`'s split/decompose coverage-judging logic -- the part S3-a's narrowed scope explicitly deferred (S3-a only extracted the detection predicate, not the judging itself). Higher risk than S5b: judging logic is subtle and has real production history of edge cases. | none |
