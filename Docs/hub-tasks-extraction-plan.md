...
| **S5a** | Generalize `queue-watcher.sh`'s plugin-script resolution (today's `dirname("${AGENT_MANAGER_REGISTER_PATH%%,*}")` hack only works for a single, first-listed plugin) so it can resolve a NAMED plugin's script path regardless of position or of whether it's listed in `AGENT_MANAGER_REGISTER_PATH` vs `plugins.json`. Lands with today's hygiene path as the only real consumer -- no behavior change. Prerequisite for S5e. | none |
