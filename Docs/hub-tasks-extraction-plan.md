...
| **S5d** | New swap point: the dashboard's `_summarize_hub`/`_hub_for_branch`/`_hub_info_for_task` (`python/dashboard/app.py`), default = today's code, tab stays hardcoded in core. Less precedent than the JS-side hooks (first Python-side swap point in this extraction). | none |
