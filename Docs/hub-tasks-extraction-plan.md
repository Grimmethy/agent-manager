...
| **S5** | Move the hub kernel: `coordinator-sweep`, `hub-serial`, `hub-rename`, `hub-restack`, `hub-status-grounding`, `decompose-auto-merge`, `decompose-integration-gate`, `rejected-hub-disposition-backfill`, `queueSubTasks`/`applyAdhocDiff` decompose branch. **Corrected 2026-09-25 (verified against the code while scoping this step):** `stacked-grounding` was wrongly listed here -- it's a generic git-ref utility with no hub logic, required by 14 unrelated files; moving it would break their dependency direction. `hub-rename`/`hub-restack` were missing from the original list (both required by `coordinator-sweep.js`); added. See `Docs/hub-tasks-extraction-map.md` section 7's own correction note. **`queue-watcher.sh`'s sweep call must change in the same commit** or the sweep silently stops -- concretely, the existing "resolve a moved sweep script through `AGENT_MANAGER_REGISTER_PATH`" trick (built for hygiene's file-decompose family) only reads the *first* comma-separated entry, and that env var currently names only `agent-manager-hygiene`; hub-tasks is registered through `plugins.json` instead, which bash cannot parse. `queue-watcher.sh` needs real per-plugin-name script resolution before any kernel sweep can move -- this is now its own piece (S5b below), not an assumption S5 can make. The Hub Tasks tab's *rendering* can move via S0's manifest-tab mechanism, but S0 has no answer yet for a Python-side data contract like `_summarize_hub`/`_hub_for_branch`/`_hub_info_for_task` (`python/dashboard/app.py`) -- recommend the tab stays hardcoded in core, reading through a new swap point, rather than a full UI move in S5. This is the one deploy window that freezes hub production; it cannot be narrowed further, only shortened by front-loading S5a-S5d below first. | S4, S0 |
| **S5a** | **Done, 2026-09-25.** Generalized plugin-script resolution: `src/resolve-plugin-root.js` (`resolvePluginRoot(name)`, reads `plugins.json` by name, falls back to a substring match against `AGENT_MANAGER_REGISTER_PATH` only when no manifest file exists), tested in `src/resolve-plugin-root.test.js`. `scripts/queue-watcher.sh`'s `resolve_plugin_root()` now calls it instead of the old `dirname("${AGENT_MANAGER_REGISTER_PATH%%,*}")` hack. Found and fixed the same bug's Python twin while here: `python/dashboard/app.py`'s proactive-file-decompose-sweep spawn (project-select route) had its own `.split(",")[0]`-only resolution -- replaced with `_resolve_plugin_root(name)`, tested in `python/dashboard/test_plugins_manifest.py`'s new `ResolvePluginRootTest`. No behavior change for today's single consumer (hygiene); verified `resolve_plugin_root agent-manager-hub-tasks` also resolves correctly now, proving the fix. Prerequisite for S5e. | none |
| **S5b** | **Done, 2026-09-25.** New swap point `src/apply-branch-prep-route.js` (`getApplyBranchPrep`/`setApplyBranchPrep`): `apply-task.js`'s `applyTask()` now dispatches its stacked-branch-vs-ordinary-branch preparation through it, default = today's inline logic moved verbatim. Same "default IS the real behavior, no unregistered state" shape as `hub-apply-routing.js` (S2) -- every apply needs a branch prepared, so unlike S4b's producer hooks there is nothing to degrade to. 8 new unit tests plus the existing `apply-task.test.js` stacked-branch suite (86/86) unchanged. | none |
| **S5c** | **Done, 2026-09-25.** New swap point `src/split-coverage-judging-route.js` (`getSplitCoverageJudging`/`setSplitCoverageJudging`): `review-task.js`'s `buildVerdictPrompt` now dispatches the original-request injection, the coverage-guidance override, and the completeness-question override through it, all moved verbatim (character-for-character checked against the incident-hardened original text). **Found, not fixed, while extracting** (recorded in the module's own header for whoever picks this up next): the coverage-guidance/completeness-question overrides key off `task.candidateSplitProposals` specifically, not the broader `isDecomposeOrSplitProposal` predicate S3-a's hook uses -- so a plain adhoc `RESOLUTION: decompose` gets the original-request injection but not the split-specific guidance text, falling through to the generic "real, complete code" question instead. Whether that's intentional is a separate question from this extraction. 10 new unit tests plus the existing `review-task.test.js` suite (86/86) unchanged. | none |
| **S5d** | **Done, 2026-09-25.** New swap point in `python/dashboard/app.py`: `_DefaultHubDataProvider` (`get_hub_data_provider`/`set_hub_data_provider`) wraps `_hub_for_branch`/`_hub_info_for_task` (`_summarize_hub` stays an internal helper of the default, not part of the interface -- it has no caller outside `_hub_for_branch`). All production call sites -- both in `app.py` itself and in `routes/instances.py` / `routes/pipeline_1_more.py`, which import the provider accessor instead of the bare functions -- now go through it. The Hub Tasks tab itself stays hardcoded in core, as the plan's own correction note above recommends; this only covers the data those routes serve it. Not the first Python-side swap point -- S5a's `_resolve_plugin_root` in `app.py` was. Circular import avoided by defining the provider inside `app.py` itself rather than a separate module (the route files already use a deferred `from app import ...` inside each view for the same reason). | none |
| **S5e** | **Done, 2026-09-25.** Moved 7 files to `agent-manager-hub-tasks`: `coordinator-sweep.js`, `hub-rename.js`, `hub-restack.js`, `decompose-auto-merge.js`, `decompose-integration-gate.js`, `rejected-hub-disposition-backfill.js`, `wire-decomposed-blueprints.js` (the last one missing from every earlier version of this list -- required only by `coordinator-sweep.js`, found while verifying against the code). **Corrected mid-implementation: `hub-serial.js`, `hub-status-grounding.js`, and `apply-adhoc-diff.js` (`queueSubTasks`/`applyAdhocDiff`) do NOT move** -- each has a core-side caller that isn't moving (`apply-adhoc-diff.js`/`hub-apply-routing.js` need `hub-serial.js`; `local-draft.js`/`local-agentic-write-draft.js` need `hub-status-grounding.js`; `task-sources.js` calls `applyAdhocDiff` directly), so moving them would violate the one-way plugin-depends-on-core rule. This is a real, load-bearing correction to the plan's original framing, not just a naming fix: "the hub kernel" that actually moves is the reconciliation sweep and its private helpers, not the filing primitive both producer 4's plugin code and core's own decompose branch call into forever. `scripts/queue-watcher.sh`'s two sweep invocations now resolve via `resolve_plugin_root` (S5a), loudly erroring (not silently skipping) if the plugin is missing, since this is not optional the way the hygiene sweeps are. **Also found and fixed**: `scripts/pool-sweeps.sh` (idle-pool borrowing housekeeping) had its own independent, unguarded sweep-invocation loop with the identical bug -- and had *already* been silently skipping 4 hygiene-family sweeps for every borrowed pool project since S4a (2026-09-24), undetected until now; fixed for both families in the same commit. **Also found and fixed**: `python/dashboard/test_hub_summary.py`'s drift-guard test hardcoded a core-relative path to `coordinator-sweep.js`; fixed to resolve the plugin by name (skips gracefully if not installed) -- which then correctly re-exposed a real, separate bug the test exists to catch: Python's `_hub_child_phase` fallback was missing the `aged-out` status (fixed: `_HUB_LEGACY_MERGED` now includes it). `hub-serial.test.js`'s one coordinator-sweep integration test moved with `coordinator-sweep.test.js` (same coverage now lives in the plugin's own test suite). | S5a, S5b, S5c, S5d, S4, S0 |
