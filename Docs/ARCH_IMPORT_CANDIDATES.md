...
### AC-13 · Explicit per-stage gate state (BLOCKED → PENDING → DONE) for the draft/review/apply chain
Strength: Strong
Source: xiaona-ai-agent-tasks — "Dependency-gated unblocking as a pipeline-stage mechanism"
Files: src/review-runner.ps1, docs/local-delegation.md, docs/adr/0017-arch-discovery-pipeline.md, src/prompts.js

Problem:
`docs/local-delegation.md` (line 827) documents that `review-runner.ps1` currently fuses review and apply into a single `claude -p` invocation, and that switching the reviewer to Ornith "necessarily means splitting review from apply into two separate steps." ADR-0017's target architecture confirms the split: `ornith-worker.ps1` → `review-runner.ps1` → `apply-runner.ps1`, with "atomic queue-state transitions" listed as existing machinery. However, the gate between stages is implicit in sequential script execution — there is no per-stage status field in the queue record that records whether a task's review stage is `DONE` or whether its apply stage is still `BLOCKED`. `src/prompts.js` (line 608) describes the pipeline as a "drafting/review/apply queue," but the queue state that `review-runner.ps1` (line 304: "the mandatory review+apply gate for a drafted task") reads and writes does not expose a stage-level enum. The result is that the dependency ("apply may only start after review approves") is enforced by process ordering rather than by data, making it invisible to the dashboard, to crash-resume logic, and to any future consumer that needs to query "which tasks are approved-but-not-yet-applied?"

Solution:
Adapt the two-step dependency-gated unblocking pattern from xiaona-ai-agent-tasks, reduced to agent-manager's fixed three-stage chain (no general `depends_on` graph, no priority sorting, no `created_at` tie-breaking). Each task record in the `agent-pipeline` queue gains a `stages` object: `{ draft: "DONE"|"PENDING"|"BLOCKED", review: "DONE"|"PENDING"|"BLOCKED", apply: "DONE"|"PENDING"|"BLOCKED" }`. At draft time (when `ornith-worker.ps1` enqueues a task), `review` and `apply` are stamped `BLOCKED`. When `review-runner.ps1` completes its majority-vote step and approves, it performs the "sweep-on-completion" step reduced to a single-predecessor check: stamp `review: "DONE"` and flip `apply` from `BLOCKED` to `PENDING` in the same atomic write that records the review verdict. `apply-runner.ps1` then selects only tasks where `stages.apply === "PENDING"`, making the gate a data-level invariant rather than a script-ordering assumption. This is the minimal form of the xiaona pattern: the "check-at-add-time" is the initial `BLOCKED` stamp, and the "sweep-on-completion" is the single `BLOCKED→PENDING` flip after the one predecessor finishes.

Benefits:
Crash-resume becomes unambiguous: if the process dies between `review-runner.ps1` finishing and `apply-runner.ps1` starting, the queue record already shows `review: "DONE", apply: "PENDING"`, so a restart can pick up exactly where it left off without re-running review. The dashboard (`python/dashboard/app.py`, which already greps for pipeline daemons) can surface a "approved, awaiting apply" column by querying the stage enum. Future task types (ADR-0017's `arch_discovery`, any new domain) inherit the gate for free — they only need to set the same three-stage enum at enqueue time, and the unblocking logic in `review-runner.ps1` is domain-agnostic. The implicit "next script in the `.bat` file" coupling is replaced by an explicit, inspectable state transition, reducing the class of bugs where a task is applied without a recorded review approval.

### AC-14 · Provenance tag on stalenessFlag confidence
Strength: Strong
Source: omnigent-ai-omnigent — "Provenance tag on every inferred value"
Files: src/adhoc-staleness-flag.js, src/adhoc-staleness-flag.test.js

Problem:
In `src/adhoc-staleness-flag.js`, the `stalenessFlag` object stamped onto a task carries a `confidence` field whose value is either `'high'` or `'medium'`, but two entirely different code paths both produce `confidence: 'high'`: (1) `classifyStaleTask` assigns high confidence from a deterministic rule (duplicate-of with a merged sibling, already-implemented-strong, invalid-premise) with no model call, and (2) the majority-vote path at lines 277–280 promotes a medium-confidence flag to high when `vote.confident && vote.verdict === 'CONFIRM'`. The test file `src/adhoc-staleness-flag.test.js` confirms both paths: the "sweep stamps a high-confidence flag with no vote" test asserts `t.stalenessFlag.confidence === 'high'` after a pure rule match, and the "CONFIRM promotes to high" test asserts the same `confidence: 'high'` after a model vote. A downstream consumer reading the task JSON (the dashboard at `python/dashboard/app.py`, the queue-watcher at `scripts/queue-watcher.sh`, or a human inspecting `queue/blocked/<id>.json`) sees `confidence: 'high'` and cannot tell whether the codebase *proved* the task is stale or a model *guessed* it is. The `reason` field partially disambiguates the rule, but the confidence level itself—the value a consumer is most likely to act on—carries no provenance.

Solution:
Add a `confidenceSource` field to the `stalenessFlag` object as a three-value enum: `'deterministic-rule'` (set in `classifyStaleTask` when it returns `confidence: 'high'` for duplicate-of / already-implemented-strong / invalid-premise), `'model-vote'` (set in the sweep's CONFIRM-promotion branch at the `vote.confident && vote.verdict === 'CONFIRM'` site, overwriting the flag's confidence to `'high'` and stamping `confidenceSource: 'model-vote'`), and `'heuristic-default'` (set when the flag is stamped at medium confidence with no vote enabled, the current default path where `majorityVote` is never called). The field is written alongside the existing `reason`, `confidence`, and `disposition` fields in the same `Object.assign` / spread that constructs the flag, so no new file or new module is needed—only the three assignment sites in `src/adhoc-staleness-flag.js` gain one extra property. In `src/adhoc-staleness-flag.test.js`, the existing assertions gain one line each: the "sweep stamps a high-confidence flag with no vote" test adds `assert.equal(t.stalenessFlag.confidenceSource, 'deterministic-rule')`; the "vote is OFF by default" test adds `assert.equal(readTask(dir, 'blocked', 'nov-1').stalenessFlag.confidenceSource, 'heuristic-default')`; and the "CONFIRM promotes to high" test adds `assert.equal(readTask(dir2, 'blocked', 'conf-1').stalenessFlag.confidenceSource, 'model-vote')`.
...
### AC-17 · Latest-decisive-verdict-per-reviewer aggregation for the majority-vote review gate
Strength: Worth exploring
Source: microsoft-agent-framework — "Latest-decisive-verdict-per-reviewer aggregation for majority-vote gate"
Files: src/review-runner.ps1, src/review-task.js, src/task-sources.js, docs/adr/0022-core-is-a-platform-plugins-define-the-work.md

Problem:
ADR 0022 commits agent-manager to a "majority-vote review gate" whose verdicts are emitted in the GUARDRAIL span vocabulary (`span.kind`, `policy.name`, `policy.phase`, `policy.action`, `policy.reason`) and whose aggregation is explicitly "deterministic (no-LLM)." In practice, `review-task.js` builds a single-pass verdict prompt via `buildVerdictPrompt` (the "You are a review gate in an unattended pipeline… VERDICT ONLY" instruction at line 176), and `review-runner.ps1` consumes that verdict to gate a real state transition (`approved -> apply-runner`, line 93). `task-sources.js` (line 1318) supplies per-source review-gate guidance that `buildVerdictPrompt` reads. However, when a task is re-reviewed after a revision—producing a second or third verdict from the same reviewer—there is no documented, deterministic rule in these files for which verdicts survive into the majority tally. The current flow appears to treat each pass as an independent single-verdict event, so a stale "reject" from pass 1 can still count alongside a fresh "approve" from pass 2, muddying the majority count and making the gate's outcome depend on pass ordering rather than on the reviewer's most recent position.

Solution:
Adapt the `latestDecisiveReviews` pattern from the source finding into agent-manager's own vocabulary. Concretely, in the aggregation step that `review-runner.ps1` performs before comparing the verdict tally to the quorum threshold (the block around line 93 where the verdict gates the `approved -> apply-runner` transition), insert a deterministic pre-filter: (1) group all verdict records for the task by reviewer identity; (2) within each group, sort by the review-pass timestamp; (3) retain only the most recent verdict whose `policy.action` is in the "decisive" set—defined for agent-manager as the GUARDRAIL span actions that actually change state (`approve`, `reject`, `escalate`), excluding non-decisive actions like `acknowledge` or `info`; (4) if a reviewer's latest verdict is non-decisive, fall back to the most recent decisive one before it. The tie-breaking rule (equal approve/reject counts after filtering) should default to the conservative outcome—block the transition and emit a `policy.reason` of `"tie"` in the GUARDRAIL span—matching the "deterministic (no-LLM)" guarantee in ADR 0022. `review-task.js`'s `buildVerdictPrompt` should be extended to tag each emitted verdict with a monotonically increasing `review.pass` integer so the aggregation step has a reliable sort key, and `task-sources.js`'s per-source guidance (line 1318) should document which `policy.action` values count as decisive for that source type so the filter is not hard-coded to a single vocabulary.

Benefits:
The majority-vote gate becomes order-independent: re-reviewing a task no longer risks a stale verdict skewing the tally, and the deterministic pre-filter satisfies the "no-LLM" aggregation promise in ADR 0022 without introducing a new runtime dependency. Reviewers get a clear contract—only their latest decisive call counts—reducing confusion when a task bounces through multiple review passes. The conservative tie-break (block on tie) aligns with the unattended-pipeline safety posture already encoded in the "VERDICT ONLY" prompt in both `review-task.js` and `review-runner.ps1`, and the per-source decisive-action list in `task-sources.js` keeps the rule extensible as new GUARDRAIL span actions are introduced.
