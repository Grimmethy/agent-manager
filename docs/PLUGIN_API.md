...
| `deterministic-recheck-registry.js` | `registerDeterministicRecheck`, `getDeterministicRecheck`, `getRecheckSources`, `clearDeterministicRecheckRegistry` | ADR-0022 Stage B. The plugin registers `{ perFileRules, repoWideRules }` per `originalSource`; core's `staleness-fastpath.js` looks them up with zero source-name knowledge. `clear` / `get*` are for the plugin's own tests. |
| `maintenance/observability-scan.js` | `scanProject`, `findSilentCatchBlocks`, `findUnguardedLoops`, `findOtelNamingViolations`, `hasOtelDependency`, `findMissingReservedAttributes` | **pure detectors stay in core** — the plugin imports them for its own `*-review.js` scan step *and* wraps them in its `registerDeterministicRecheck` config so `staleness-fastpath.js` can re-run a rule against a file's current content (see `src/maintenance/README.md`). |
