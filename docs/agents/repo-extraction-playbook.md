...
| Target | Status | Doc |
|---|---|---|
| Hub Tasks → `agent-manager-hub-tasks` | S0 done (PR #455). S1 (claim-ordering/dependency-release hooks, PR #459), S2 (apply-result routing hook, PR #460), S3-a (decompose/split-proposal detection hook, narrowed from full S3, PR #461) all merged 2026-09-23. S4a done 2026-09-24 (file-decompose family moved to hygiene, PR #468). **S4b done, 2026-09-25**: repo scaffolded, then both task-level producers moved -- producer 4 (candidate split, `candidate-split-hub-route.js`) and producer 1 (adhoc decompose, `decompose-pass-route.js`; the latter's swap point degrades gracefully rather than throwing, since it's on the hot path for every adhoc draft — see the Worktrees section's incident note). Their caller-side dispatch logic stays in core. Hub-intake hook shape decided (section 3) but the fuller generalization deferred to S5. **S5 scoped 2026-09-25 (read-only)**: corrected two real plan errors (`stacked-grounding.js` wrongly listed as kernel code -- it's a 14-file-shared generic utility; `hub-rename.js`/`hub-restack.js` missing from the list) and split S5 into S5a-e (queue-watcher.sh plugin-resolution generalization, three new swap points for apply-task.js/review-task.js/the dashboard's hub-data functions, then the actual kernel move + freeze window). **S5a done, 2026-09-25**: `src/resolve-plugin-root.js` generalizes plugin-script resolution by name (also fixed the same bug's Python twin in `app.py`'s proactive-sweep spawn). **S5b done, 2026-09-25**: `src/apply-branch-prep-route.js` -- a new swap point for `apply-task.js`'s stacked-branch preparation, same "always-installed default" shape as `hub-apply-routing.js`. **S5c done, 2026-09-25**: `src/split-coverage-judging-route.js` -- a new swap point for `review-task.js`'s decompose/split coverage-judging text, moved verbatim (checked character-for-character against the incident-hardened original wording). Found (not fixed) a pre-existing inconsistency while extracting: the override keys off `task.candidateSplitProposals` specifically, not the broader detection predicate, so a plain adhoc decompose falls through to generic completeness wording -- recorded in the module's own header. **S5d done, 2026-09-25**: `python/dashboard/app.py`'s `_DefaultHubDataProvider` -- the first Python-side dashboard swap point in this extraction (S5a's `_resolve_plugin_root` was the first Python swap point overall), wrapping `_hub_for_branch`/`_hub_info_for_task`; both `app.py`'s own call sites and the two route files (`routes/instances.py`, `routes/pipeline_1_more.py`) now go through it. Circular import avoided by keeping the provider inside `app.py` rather than a new module. **S5e done, 2026-09-25**: moved 7 files (`coordinator-sweep.js`, `hub-rename.js`, `hub-restack.js`, `decompose-auto-merge.js`, `decompose-integration-gate.js`, `rejected-hub-disposition-backfill.js`, `wire-decomposed-blueprints.js`) behind S5a-d's hooks; `queue-watcher.sh`'s two sweep invocations flipped to `resolve_plugin_root`, erroring loudly (not silently skipping) if the plugin is missing. **Real load-bearing correction found while moving files, not just a naming fix**: `hub-serial.js`, `hub-status-grounding.js`, and `apply-adhoc-diff.js`'s `queueSubTasks` do NOT move -- each has a core-side caller that isn't moving, so "the hub kernel" that actually moves is the reconciliation sweep and its private helpers, never the filing primitive core's own decompose branch calls into forever. Also found and fixed two separate pre-existing/newly-exposed bugs in the same commit: `scripts/pool-sweeps.sh` had its own independent, unguarded sweep-invocation loop that had silently skipped the S4a hygiene-family sweeps for every borrowed pool project since 2026-09-24, undetected until now; and `test_hub_summary.py`'s drift-guard test (once repointed at the plugin) correctly re-exposed that Python's `_hub_child_phase` fallback was missing the `aged-out` status. S3's `verifyMove` hook, S6 not started. | `Docs/hub-tasks-extraction-plan.md` |
