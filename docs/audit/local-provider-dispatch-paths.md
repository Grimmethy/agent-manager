...
| 5 | `python/dashboard/app.py` `_run_build` -> `python/graph_build.py`; also the queue-watcher's daily rebuild and deep_dive onboarding via `python/build_graph.py` | `_run_build` background thread defined at `python/dashboard/app.py:2606` calls `graph_build.build_graph_data(...)` at `python/dashboard/app.py:2634`, whose community-naming step POSTs `{ollama_url}/api/generate` at `python/graph_communities.py:83`. The daily `--check-due` rebuild (`scripts/queue-watcher.sh:513`) and deep_dive lead onboarding (`src/task-sources.js:892`) still run `python/build_graph.py` (POST at `python/build_graph.py:512`, URL default at `:154`) until that file is retired (HUB0022) | No — background thread / subprocess, not a chat route | Background third path |
| 6 | `src/` worker lanes (e.g. `src/local-draft.js`) | `src/local-draft.js` dispatches its plan/implement passes through `resolvedLocalCall(...)` (e.g. `src/local-draft.js:635`, `:975`), which resolves to `local-client.js`'s `call()` (`src/local-client.js:459`) | No — these lanes are the *victims* killed by the preempt, not preemptors | Preempted victims |
...
- **Background dispatcher:** `app.py _run_build` → `python/graph_build.py` (and the
  daily rebuild / deep_dive onboarding via `python/build_graph.py`) (row 5) — fires Ollama work on its own thread with no chat context at all.
