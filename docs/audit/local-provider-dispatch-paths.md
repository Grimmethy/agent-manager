...
| 5 | `python/dashboard/app.py` `_run_build`, the queue-watcher's daily rebuild, and deep_dive onboarding -- all via `python/graph_build.py` | `_run_build` background thread defined at `python/dashboard/app.py:2606` calls `graph_build.build_graph_data(...)` at `python/dashboard/app.py:2634`; the daily `--check-due` rebuild (`scripts/queue-watcher.sh:514`) and deep_dive lead onboarding (`src/task-sources.js:892`) run the same module as a subprocess. Its community-naming step POSTs `{ollama_url}/api/generate` at `python/graph_communities.py:83` | No — background thread / subprocess, not a chat route | Background third path |
...
- **Background dispatcher:** `app.py _run_build` → `python/graph_build.py`, also the
  daily rebuild and deep_dive onboarding (row 5) — fires Ollama work on its own thread with no chat context at all.
