...
function buildDebriefConfirmPrompt(task) {
  ...
  return [
    'You are the confirmation gate for a pipeline_debrief DETERMINISM AUDIT (WHAT / SO WHAT /',
    'NOW WHAT) report. The report has ALREADY passed a review that judged it well-formed.',
    'Your ONLY decision now: is it safe to (1) archive this window\'s done/ tasks, and',
    '(2) file its NOW WHAT items into the brain-dump inbox for further triage?',
    '',
    `TITLE: ${task.title || '(none)'}`,
    ...
    '  real-looking model-call counts/timings/attempt counts, a NOW WHAT that (if present)',
    '  names real src/ files and a concrete deterministic mechanism -- even if the finding',
    '  is modest. A short, narrow, well-evidenced audit is as valid as a dramatic one.',
    'DENY if: the report is "NO CONFIDENT INEFFICIENCY" with no real content around it; it',
    '  invents a file/symbol/task id not plausibly from the evidence; it contradicts itself',
    '  (e.g. claims an ALREADY-DETERMINISTIC CHECK it never actually did); it re-proposes',
    '  something the fetched file content shows already exists; or NOW WHAT is vague',
    '  hand-waving with no real file or mechanism cited.',
    '',
    'If you genuinely cannot decide, do not force it -- a non-answer leaves it for a human.',
    'Answer with EXACTLY one line, nothing after it:',
