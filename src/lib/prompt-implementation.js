...
function brainDumpSortImplementPrompt(task, planText) {
  ...
  return [
    ...
    '  "requiresResearch": true or false -- true ONLY if properly resolving this note means finding NEW information from EXTERNAL sources (the public web, another project\'s own docs/repo/GitHub issues, a library\'s release notes) -- never true for a note that just needs THIS repo\'s own code, config, or queue state read/confirmed, however it is phrased ("confirm before attributing", "should be verified", "check whether X is true"). That kind of internal verification is not research: an ordinary adhoc task can already grep this repo directly, with no web access at all, and research_task tasks are NOT guaranteed to have web access enabled on every deployment -- so a false-positive here can strand the note somewhere it can never actually get resolved. Independent of belongsToProject -- this is never a code change, and should never be true at the same time as naming a belongsToProject.',
