...
// 2026-09-20: one pinned context for every call. The old "extended" tier (a bigger num_ctx for whole-document sources, bought by evicting the
// small utility model) is gone with the small model; a num_ctx that differs between calls makes Ollama reload the 27B (~55-100s).
test('computeImplementBudget always returns the one pinned implNumCtx, even for a whole-document source with a huge prompt', () => {
  const { computeImplementBudget } = require('./local-draft.js');
  const { PINNED_NUM_CTX } = require('./gpu-capacity.js');
  const big = computeImplementBudget({ source: 'product_spec', planResponse: 'y'.repeat(9000) }, 'z'.repeat(80000));
  const small = computeImplementBudget({ source: 'manual', planResponse: 'x'.repeat(1500) }, 'a normal implement prompt '.repeat(120));
  assert.equal(big.implNumCtx, PINNED_NUM_CTX);
  assert.equal(small.implNumCtx, PINNED_NUM_CTX);
});
...
// The require.main CLI-guard regression test that used to live here (2026-09-14,
// screaminggoatclubmt) moved to agent-manager-hygiene/src/decompose-node-module.test.js
// with decompose-node-module.js itself (S4a of the hub-tasks extraction, 2026-09-24) --
// it tests buildNodeModuleOnePassChanges's own parsing directly there now, which this
// file's local-draft.js integration path (blocked from loading the real hygiene plugin by
// neutralizeRealPluginLoading() above, deliberately, to keep this suite hermetic) can no
// longer exercise end to end without depending on the plugin being installed.
