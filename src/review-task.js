...
function buildVerdictPrompt(task, factCheck, groundingText) {
  ...
  lines.push('The fact-check above is deterministic and authoritative for file existence -- it already checked the real filesystem. A claimed path listed with "exists": true is CONFIRMED real; do not express doubt about it or re-litigate whether it exists. Only "exists": false (a missing-file flag) is evidence toward fabrication.');
  lines.push('');
