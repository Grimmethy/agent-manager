...
function brainDumpSortPlanPrompt(task) {
  ...
  return [
    ...
    '',
    'Think through, in a short numbered list: (1) what this note is actually about, (2) whether it is a task/reminder that needs someone to DO something, or just something to remember/reference, (3) which existing folder (or a new one, only if genuinely nothing fits) it belongs under, (4) a short relative file path within that folder to file it under (an existing note to append to, or a new one to create), (5) if this describes a concrete feature/bug IN one of the tracked code projects listed above (i.e. an edit to that project\'s own existing files), name which one -- otherwise say none apply, and (6) if properly resolving this note means going out and finding NEW EXTERNAL information first (something on the public web -- a product, service, account, business, or public event) rather than just filing the note as stated, say so -- that makes it a real research task, independent of (5) (a research task is never a code change, and it never has real access to any tracked project\'s own repo -- see the CRITICAL note just below).',
    '',
