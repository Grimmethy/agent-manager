...
const candidateSplitInstructions = [
  ...
  '  {"title": "...", "files": "comma, separated, paths", "problem": "...", "solution": "...", "benefits": "...", "dependsOn": null},',
  '  {"title": "...", "files": "comma, separated, paths", "problem": "...", "solution": "...", "benefits": "...", "dependsOn": 0}',
  ']}',
  '',
  ...
  '',
  '"dependsOn": if a sub-candidate can only be implemented correctly AFTER an earlier one in this SAME array has actually landed (e.g. it reads a field/function that earlier one creates), set it to that earlier candidate\'s 0-based position in the array (0 for the first candidate, 1 for the second, etc). Use null when a sub-candidate is independent -- do NOT chain unrelated candidates together just to be safe. A later sub-candidate naming a dependency will not be offered for drafting until the one it depends on has actually merged, so use this whenever the two are genuinely ordered, not just prose ("previously", "the AC-13a gate") that nothing downstream ever enforces.',
  '',
