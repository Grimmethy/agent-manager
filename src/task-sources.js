...
const ADHOC_GROUNDED_PLAN_GUIDANCE = 'The PLAN section above was drafted with PARTIAL file access -- a deterministic grep plus the specific files the task names -- before the full agentic implementation. You MAY weigh whether the PLAN tracked the task\'s stated requirements, but still judge PRIMARILY the IMPLEMENT draft below (produced after real investigation), and NEVER reject solely over rough or broken illustrative code in the PLAN.';
...
const ADHOC_DECOMPOSE_GUIDANCE = 'This is an adhoc task the drafter chose to DECOMPOSE rather than implement directly: it judged the task too large/broad to implement confidently in one pass and produced a JSON array of smaller sub-tasks instead of a diff -- there is deliberately no code or diff here, and that is NOT a reason to reject. Judge ONLY the actual DECOMPOSITION in the IMPLEMENT draft below. COVERAGE IS THE MAIN TEST: take the ORIGINAL REQUEST shown above and enumerate every concrete deliverable it names -- each endpoint, function, config knob, file, UI element, behavior. For EACH one, point at the specific sub-task that delivers it. If even one named deliverable is not covered by any sub-task, REJECT -- a decompose that keeps the peripheral pieces (a data file, a test) but drops the core change (the endpoint/function the task is actually about) is the single most common failure of this path and must not be approved. Then: is each sub-task concrete and independently implementable on its own (not still vague, not itself obviously too large)? Is the JSON well-formed with a real title and a self-contained rawText for each entry? Reject if a deliverable was dropped, a sub-task is too vague/large to actually help, or the JSON itself is malformed -- never because of something in the PLAN, and never merely because no code was written. NOTE: if the draft summary says an earlier pass already implemented part of the work, that partial work is NOT carried forward -- the sub-tasks must still cover it.';
...
// 2026-09-12 (screaminggoatclubmt, from the single biggest identifiable sub-pattern in a
// 31-task pipeline_debrief blocked cluster): the drafting prompt (pipelineDebriefPlanPrompt/
// buildImplementPrompt in prompts.js) explicitly tells the model "If AVAILABLE FILES is
// empty or none fit, omit the Files: line and recommend in prose. A recommendation naming
// no file is far better than one naming a wrong one" -- but this review guidance's own
// reject list never carried the same exception, so the reviewer rejected drafts for
// correctly doing exactly what the drafting prompt told them to do. Confirmed live against
// 5+ real blocked instances whose promptContext.availableFiles was genuinely empty and
// whose draft said so verbatim ("AVAILABLE FILES is empty, so no path may be cited",
// "(None applicable from AVAILABLE FILES; recommend in prose)") yet still got rejected
// under "NOW WHAT is vague/unbounded with no real file cited." Fix mirrors the drafting
// prompt's own escape hatch back into the review side, rather than tightening the
// drafting prompt (which was already correct) or loosening it further (which would let a
// draft with REAL available files skip citing one).
const PIPELINE_DEBRIEF_REVIEW_GUIDANCE = 'This is a pipeline_debrief task: the implement draft is DELIBERATELY an advisory prose report -- a DETERMINISM AUDIT (WHAT / SO WHAT / NOW WHAT) over a WINDOW of pipeline tasks that already SHIPPED, finding model calls spent deciding something a deterministic check/transform/lookup/existing-gate could have decided -- not code or a diff. There is nothing to implement here. Judge it on: (1) does WHAT give a per-task/cluster model-call profile grounded in the evidence (real stages, call counts, attempt counts -- not invented)? (2) does SO WHAT flag SPECIFIC model calls as mechanically decidable, each with a cited task id + history stage + model_calls row, rather than a generic platitude? (3) does the ALREADY-DETERMINISTIC CHECK actually check the AVAILABLE FILES content for an existing gate/normalizer before proposing one -- classifying each as not-wired-here / stale / genuinely absent -- rather than skip it? (4) is NOW WHAT at most 2-3 concrete, bounded items, each naming a stage + an exact deterministic mechanism + rough calls-saved, citing a real src/ file from AVAILABLE FILES when one genuinely fits (not an invented module name, not a wish list)? If AVAILABLE FILES was empty, or none of the listed files actually fit the recommendation, omitting the Files: line and recommending in prose is the CORRECT, REQUIRED behavior per the drafting instructions -- do NOT reject a draft for doing this; only reject if a real, relevant file WAS available and the draft skipped citing it anyway. "NO CONFIDENT INEFFICIENCY" is a valid, correct outcome when the evidence genuinely shows none -- do NOT reject it under the hedging rule. Reject only if: it invents a file/symbol not in the evidence, it contradicts the evidence (claims a task/history entry says something the record does not), it skips the ALREADY-DETERMINISTIC CHECK when AVAILABLE FILES were provided, it re-proposes something already present in the fetched file content, or NOW WHAT is vague/unbounded AND a real, relevant file from AVAILABLE FILES existed but was not cited.';
const PIPELINE_DEBRIEF_COMPLETENESS_QUESTION = 'Does it contain all four sections (WHAT, SO WHAT, ALREADY-DETERMINISTIC CHECK, NOW WHAT) or a justified NO CONFIDENT INEFFICIENCY, with SO WHAT flagging specific model calls as mechanically decidable (cited) and NOW WHAT (if present) bounded to 2-3 items each naming a stage + deterministic mechanism + a real src/ file when one genuinely fits (omitting the Files: line in prose is correct and complete when AVAILABLE FILES was empty or none fit) -- all grounded in the real evidence rather than invented?';
...
function adhocReviewGuidance(task) {
  ...
  const planGuidance = task.planWasGrounded ? ADHOC_GROUNDED_PLAN_GUIDANCE : ADHOC_BLIND_PLAN_GUIDANCE;
  return `${planGuidance}\n${body}`;
}
